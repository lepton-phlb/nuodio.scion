...
int nu_display_main_app(struct xlcd_context_st* xlcd_context,
   ...
   switch (msgId) {
      case MSG_DISPLAY_INIT: {
         //distortion type
         ioctl(p_nu_xgui->fd_rotary_1, ENCODERSETCOUNTER, p_nu_xgui->distortion_type);
         ioctl(p_nu_xgui->fd_rotary_1, ENCODERSETCOUNTERLIMIT, DIST_TYPE_MIN, DIST_TYPE_MAX);
         //sat
         ioctl(p_nu_xgui->fd_rotary_2, ENCODERSETCOUNTER, p_nu_xgui->sat_value);
         ioctl(p_nu_xgui->fd_rotary_2, ENCODERSETCOUNTERLIMIT, SAT_VALUE_MIN, SAT_VALUE_MAX);
         //tone
         ioctl(p_nu_xgui->fd_rotary_3, ENCODERSETCOUNTER, p_nu_xgui->tone_value);
         ioctl(p_nu_xgui->fd_rotary_3, ENCODERSETCOUNTERLIMIT, TONE_VALUE_MIN, TONE_VALUE_MAX);
         //level
         ioctl(p_nu_xgui->fd_rotary_4, ENCODERSETCOUNTER, p_nu_xgui->level_value);
         ioctl(p_nu_xgui->fd_rotary_4, ENCODERSETCOUNTERLIMIT, LEVEL_VALUE_MIN, LEVEL_VALUE_MAX);
      ...
      case __XLCD_IO_EVENT: {
         ...
         if (FD_ISSET(p_nu_xgui->fd_rotary_1, &xlcd_context->readfs)) {
            if ((cb = read(p_nu_xgui->fd_rotary_1, &counter, sizeof(counter))) == sizeof(counter)) {
               p_nu_xgui->distortion_type = counter;
               guiModifyBitmap(ID_SATURATION_TYPE, (void*)distorsion_type_label_image_list[p_nu_xgui->distortion_type], NULL);
            }
         }
         //sat
         if (FD_ISSET(p_nu_xgui->fd_rotary_2, &xlcd_context->readfs)) {
            if ((cb = read(p_nu_xgui->fd_rotary_2, &counter, sizeof(counter))) == sizeof(counter)) {
               p_nu_xgui->sat_value = counter;
               guiModifyBitmap(ID_SATURATION_SAT, (void*)distorsion_type_label_number_list[p_nu_xgui->sat_value], NULL);
            }
         }
         //tone
         if (FD_ISSET(p_nu_xgui->fd_rotary_3, &xlcd_context->readfs)) {
            if ((cb = read(p_nu_xgui->fd_rotary_3, &counter, sizeof(counter))) == sizeof(counter)) {
               p_nu_xgui->tone_value = counter;
               guiModifyBitmap(ID_SATURATION_TONE, (void*)distorsion_type_label_number_list[p_nu_xgui->tone_value], NULL);
            }
         }
         //level
         if (FD_ISSET(p_nu_xgui->fd_rotary_4, &xlcd_context->readfs)) {
            if ((cb = read(p_nu_xgui->fd_rotary_4, &counter, sizeof(counter))) == sizeof(counter)) {
               p_nu_xgui->level_value = counter;
               guiModifyBitmap(ID_SATURATION_LEVEL, (void*)distorsion_type_label_number_list[p_nu_xgui->level_value], NULL);
            }
         }
         break;
